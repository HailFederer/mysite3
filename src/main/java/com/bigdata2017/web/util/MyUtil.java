...
// Get 방식에 의한 페이지 처리 메서드 *************************************
    public String pageIndexList(int current_page, int total_page, String list_url) {
        ...
        if ((total_page > numPerBlock) && (currentPageSetUp > 0)) {
           strList.append("<a href='"+list_url+"pageNo=1'>1</a> ");
        }

        // [Prev] : 총 페이지수가 numPerBlock 이상인 경우 이전 numPerBlock 보여줌
        n = current_page - numPerBlock;
        if (total_page > numPerBlock && currentPageSetUp > 0) {
           strList.append("[<a href='"+list_url+"pageNo="+n+"'>Prev</a>] ");
        }
        ...
        while((page <= total_page) && (page <= currentPageSetUp + numPerBlock)) {
           if(page == current_page) {
             ...
             strList.append("<a href='"+list_url+"pageNo="+page+"'>"+page+"</a> ");
           }
           page++;
        }
        ...
        if (total_page - currentPageSetUp > numPerBlock) {
			strList.append("[<a href='"+list_url+"pageNo="+n+"'>Next</a>] ");
        }

        // 마지막 페이지
        if ((total_page > numPerBlock) && (currentPageSetUp + numPerBlock < total_page)) {
			strList.append("<a href='"+list_url+"pageNo="+total_page+"'>"+total_page+"</a>");
        }
